...
describes what users get. Pick a format with `-o`, or let sdss choose:
a table when writing to a terminal, JSONL when piped.

## Formats

```bash
sdss <command> -o table   # aligned columns for reading
sdss <command> -o jsonl   # one JSON object per line, for piping
sdss <command> -o json    # a single JSON array
sdss <command> -o csv     # spreadsheet friendly
sdss <command> -o tsv     # tab-separated
sdss <command> -o url     # just the URL column
sdss <command> -o raw     # the underlying bytes, unformatted
```
...
```bash
sdss <command> --fields id,title,url
```
...
```bash
sdss <command> --template '{{.URL}} {{.Title}}'
```
...
```bash
sdss <command>            # a table, because this is a terminal
sdss <command> | wc -l    # JSONL, because this is a pipe
```
